...
function saveContent(content) {
  ...
  localStorage.setItem(STORAGE_KEY, contentSnapshot);
  const firestore = getDb();
  if (firestore) {
    firestore.collection("galewela-site").doc("content").set({
      ...content,
      updatedAt: firebase.firestore.FieldValue.serverTimestamp()
    }).catch(console.error);
  }
...
function renderDetails(content) {
  ...
  if (detailsForm.elements.phone) detailsForm.elements.phone.value = content.details.phone || "";
  if (detailsForm.elements.email) detailsForm.elements.email.value = content.details.email || "";
  if (detailsForm.elements.address) detailsForm.elements.address.value = content.details.address || "";
  if (detailsForm.elements.welcome) detailsForm.elements.welcome.value = content.details.welcome || "";
}
...
function renderStats(content) {
  ...
  if (detailCountSpan) detailCountSpan.textContent = Object.values(content.details).filter(Boolean).length;
...
async function getChatMessagesForBackup() {
  try {
    ...
    const snapshot = await firestore.collection("galewela-public-chat").orderBy("createdAt", "desc").limit(200).get();
    return snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })).reverse();
  } catch (error) {
    console.error(error);
    try { return JSON.parse(localStorage.getItem("galewela-chat-messages") || "[]"); } catch { return []; }
  }
}
...
async function importFullBackup(file) {
  ...
  if (backup.content) localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...defaultContent, ...backup.content }));
  if (backup.bookings) localStorage.setItem(BOOKINGS_KEY, JSON.stringify(backup.bookings));
  if (backup.chat) localStorage.setItem("galewela-chat-messages", JSON.stringify(backup.chat));
  const firestore = getDb();
  if (firestore) {
...
document.querySelectorAll(".profile-form[data-profile]").forEach((form) => {
  form.addEventListener("submit", async (event) => {
    ...
    content.profiles[key] = {
      image: image || (content.profiles[key] ? content.profiles[key].image : "") || "",
...
document.querySelectorAll(".profile-form[data-officer]").forEach((form) => {
  form.addEventListener("submit", async (event) => {
    ...
    content.officers[key] = {
      image: image || (content.officers[key] ? content.officers[key].image : "") || "",
      name: form.elements.name.value.trim(),
...
if (galleryForm) {
  galleryForm.addEventListener("submit", async (event) => {
    ...
    const src = await fileToDataUrl(formData.get("image"));
    const content = getContent();
...
if (resetBtn) {
  resetBtn.addEventListener("click", () => {
    ...
    const firestore = getDb();
    if (firestore) firestore.collection("galewela-site").doc("content").set(defaultContent).catch(console.error);
    render();
  });
}
...
window.addEventListener("storage", (event) => {
  ...
  if (event.key === BOOKINGS_KEY) renderBookings();
});
...
function bootFirebaseSync() {
  ...
  firestore.collection("galewela-site").doc("content").onSnapshot((doc) => {
    ...
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...defaultContent, ...data }));
    render();
  }, console.error);
